...
CSV = "1838_FINAL_lhs_results_summary.csv"
OUT_DIR = Path("plots")
...
def fig_combined_overview_and_raincloud(df):
    ...
    ax1.set_xticklabels(df["config_name"].str.replace(r"NEW_LHS_1838_|Type\d*_", "", regex=True),rotation=55,ha="right",fontsize=8,)

    ax1.set_ylabel("Robustness Score (in %)", fontsize=12.5, labelpad=10)
    ax1.set_title("Latin Hypercube Sampling Results, 1 million simulations\nRobustness of different diffusion rate configurations for Topology #1838",fontsize=14,loc="center",pad=10,)
    ...
    ax2.set_title("Robustness distribution by Type for Topology #1838",fontsize=14,loc="center",pad=10,)
