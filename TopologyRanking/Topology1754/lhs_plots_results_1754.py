...
#### scatter plot shaberi vs diego robustness score across type 1 to 3 (will not include anymoreeee)
...
def fig_combined_overview_and_raincloud(df):
    ...
    for i, (t, label) in enumerate(zip(types, labels)):
        ...
        y_range = np.linspace(subset.min() - subset.std()*0.3, subset.max() + subset.std()*0.3, 200) # this will not work for all topologies bc 18xx robustness is sooo low
        ...
        for val in subset:
            ...
            marker = ("^" if any("Unequal" in row["config_name"] for _, row in df[(df["turing_type"] == t) & (df["rob_shaberi_total"] == val)].iterrows()) else "o")
