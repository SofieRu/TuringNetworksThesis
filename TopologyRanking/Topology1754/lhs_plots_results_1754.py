...
def fig4_diego_vs_shaberi(df):
    ...
    fig.tight_layout()
    save(fig, "new_1754_lhs_fig4_diego_vs_shaberi")
...
def fig_combined_overview_and_raincloud(df):
    ...
    ax1.set_xlabel("ID of Diffusion Configurations", fontsize=14)

    ax1.set_ylabel("Robustness Score (in %)", fontsize=14, labelpad=10)
    ax1.set_title("Latin Hypercube Sampling Results, 1 million simulations\nRobustness of different diffusion rate configurations for Topology #1754",fontsize=17,loc="center",pad=10,)
    ...
    ax2.set_title("Robustness distribution by Type for Topology #1754",fontsize=17,loc="center",pad=10,)
    ...
    fig.legend(handles=bar_handles,frameon=False,loc="lower center",bbox_to_anchor=(0.3, 0.02),ncol=3,fontsize=14) # title="Turing Type"
    fig.legend(handles=rain_handles,frameon=False,loc="lower center",bbox_to_anchor=(0.7, 0.02),ncol=2,fontsize=14) # title="Diffusion Variant",
...
#     ax.set_title(
#         "Robustness of Topologies for #1754\n(Latin Hypercube Sampling, 1 million simulations)",
#         fontsize=12, loc="left", pad=10,
#     )
...
#     fig.tight_layout()
#     save(fig, "new_1754_lhs_fig1_overview_bar_detail")
...
# #     fig.tight_layout()
# #     save(fig, "new_1754_lhs_fig2_dotplot")
...
# #         y_range = np.linspace(subset.min() - subset.std()*0.1, subset.max() + subset.std()*0.1, 200)
# #         kde_vals = kde(y_range)
...
# #         mean_val = subset.mean()
# #         kde_at_mean = kde(mean_val)[0]
# #         kde_at_mean = kde_at_mean / kde_vals.max() * 0.35 # Exact width of the cloud at the mean
# #         # Draw the line starting from the cloud's left edge up to its right edge (i)
# #         ax.hlines(mean_val, i - kde_at_mean, i, color="black", linewidth=1.5, zorder=4)
...
# #     fig.tight_layout()
# #     save(fig, "new_1754_lhs_fig2_raincloud")
