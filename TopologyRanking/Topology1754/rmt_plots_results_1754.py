...
#     # # ---> FIX FOR 10^X FORMATTING HERE <---
#     # ax_stable.ticklabel_format(axis='y', style='sci', scilimits=(0,0))
#     # ax_stable.yaxis.get_offset_text().set_fontsize(10)
...
#     # ---> FIX FOR ADVANCED SCIENTIFIC NOTATION <---
...
#         # Using a clean format string with an integer cast for the base multiplier
#         return f"${int(base)} \\times 10^{{{exponent}}}$"

#     ax_stable.yaxis.set_major_formatter(mticker.FuncFormatter(sci_notation))
#     ax_stable.xaxis.grid(False)
...
#     fig.suptitle("Random Matrix Theory Results, 1 million simulations\nRobustness of different diffusion rate configurations for Topology #1754", fontsize=13, y=0.98)
...
def complete_robustness_figure(df, sigma_val):
    ...
    fig.legend(handles=global_handles, frameon=False, loc='lower center', bbox_to_anchor=(0.5, 0.01), ncol=6, fontsize=12, handlelength=2.4, columnspacing=1.6,)
    fig.suptitle('Random Matrix Theory Results, 1 million simulations\n' 'Robustness of different diffusion rate configurations for Topology #1754', fontsize=17, y=0.98,)
