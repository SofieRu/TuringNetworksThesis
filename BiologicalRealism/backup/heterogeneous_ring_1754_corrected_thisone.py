...
# TOPOLOGY 1754: node u does NOT self-activate (15 params). 3954 = same file with
# u self-activation added (16 params).
...
#    step. This tracks the near-uniform branch robustly; a discard means the branch
...
#     alpha_w, beta_w, K_ww, K_uw, K_vw, delta_w = params[9:15]
#     J = np.zeros((3, 3))
#     # --- row u: 1754 has NO self-activation (differs from 3954) ---
#     J[0, 0] = -delta_u
#     J[0, 1] = beta_u * dH_inh(v, K_vu)
#     J[0, 2] = 0
#     # --- rows v, w: identical to 3954 ---
...
#     back on success. None => the near-uniform branch genuinely folds away."""
...
#     baseline_params = np.array([
#         row['alpha_u'], row['beta_u'], row['K_vu'], row['delta_u'],            # u: 4 (no K_uu)
...
# --- Global Kinetic & Hill Configurations ---
n = 2
...
def single_cell_rhs(state, params):
    u, v, w = state
    alpha_u, beta_u, K_vu, delta_u = params[0:4]
    alpha_v, beta_v, K_uv, K_wv, delta_v = params[4:9]
    alpha_w, beta_w, K_ww, K_uw, K_vw, delta_w = params[9:15]
...
# --- Full Network Coupled System Equations ---
def full_ring_rhs(full_state, N_cells, params_list, hopping):
    """Calculates the derivative of all cells simultaneously, including real-time hopping."""
    h = np.array([hopping["h_u"], hopping["h_v"], hopping["h_w"]])
    derivs = np.zeros_like(full_state)
    
    for i in range(N_cells):
        idx = 3 * i
        left, right = (i - 1) % N_cells, (i + 1) % N_cells
        
        # Reaction terms for cell i
        cell_state = full_state[idx:idx+3]
        derivs[idx:idx+3] = single_cell_rhs(cell_state, params_list[i])
        
        # Diffusion (hopping) terms for cell i
        left_state = full_state[3*left:3*left+3]
        right_state = full_state[3*right:3*right+3]
        derivs[idx:idx+3] += h * (left_state + right_state - 2 * cell_state)
        
    return derivs

def find_coupled_ring_steady_state(N_cells, params_list, hopping, baseline_ss):
    """Finds a valid global steady state for the entire connected ring simultaneously."""
    initial_guess = np.tile(baseline_ss, N_cells)
    
    # Attempt to solve the 3*N system directly
    ss, info, ier, msg = fsolve(full_ring_rhs, initial_guess, args=(N_cells, params_list, hopping), full_output=True)
    if ier == 1 and np.max(np.abs(full_ring_rhs(ss, N_cells, params_list, hopping))) < 1e-8 and np.all(ss > 0):
        return ss
        
    # Multi-start fallback if the homogeneous state guess fails
    for _ in range(50):
        random_guess = np.random.uniform(0.01, 10.0, 3 * N_cells)
        ss, info, ier, msg = fsolve(full_ring_rhs, random_guess, args=(N_cells, params_list, hopping), full_output=True)
        if ier == 1 and np.max(np.abs(full_ring_rhs(ss, N_cells, params_list, hopping))) < 1e-8 and np.all(ss > 0):
            return ss
    return None
...
def continuous_discrete_dispersion(J_local, N_cells, hopping):
    h = np.array([hopping["h_u"], hopping["h_v"], hopping["h_w"]])
    max_reals = []
    for m in range(N_cells // 2 + 1):
        lambda_m = -4 * np.sin(np.pi * m / N_cells)**2
        J_m = J_local + np.diag(lambda_m * h)
        max_reals.append(np.max(np.real(np.linalg.eigvals(J_m))))
    return np.array(max_reals)
...
def build_ring_jacobian_heterogeneous_coupled(N_cells, baseline_params, hopping, CV, baseline_ss):
    """Generates random cell parameters, finds the absolute true coupled fixed point,
    and returns the accurate heterogeneous ring Jacobian system."""
    sigma = np.sqrt(np.log(1 + CV**2))
    mu = -sigma**2 / 2
    params_list = []
    ...
    # CRITICAL UPDATE: Find the true unified steady state of the interconnected ring
    x_star_coupled = find_coupled_ring_steady_state(N_cells, params_list, hopping, baseline_ss)
    if x_star_coupled is None:
        return None, "no_coupled_ss", None, None
        
    Ldiff = build_diffusion_operator(N_cells, hopping)
    J_ring = Ldiff.copy()
    
    # Build accurate linearized Jacobian using states calculated from the coupled fixed point
    for i in range(N_cells):
        idx = 3 * i
        cell_ss = x_star_coupled[idx:idx+3]
        J_ring[idx:idx+3, idx:idx+3] += compute_jacobian(cell_ss, params_list[i])
        
    balance_resid = np.linalg.norm(Ldiff @ x_star_coupled) / np.linalg.norm(x_star_coupled)
    return J_ring, x_star_coupled, params_list, balance_resid
...
if __name__ == "__main__":
    ...
    # 1. Baseline Analysis (CV = 0)
    J_local_baseline = compute_jacobian(steady_state_expected, baseline_params)
    disp0 = continuous_discrete_dispersion(J_local_baseline, N_cells, hopping)
    is_turing_baseline = (disp0[0] < 0) and (np.max(disp0[1:]) > 0)
    
    print("=" * 70)
    print(f"Discrete N={N_cells} baseline: m=0 {disp0[0]:+.4f}, max(m>0) {np.max(disp0[1:]):+.4f}")
    print(f"Is Baseline Homogeneous Ring Turing Unstable? -> {is_turing_baseline}")
    print("=" * 70)

    # 2. Heterogeneity Parameter Loop
    np.random.seed(42)
    results_by_cv = []
    ...
    for CV in [0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4]:
        ...
        for _ in range(n_trials):
            if CV == 0:
                ...
                reaction_max = np.max(np.real(np.linalg.eigvals(J_local_baseline)))
                balance_resid = 0.0
            else:
                # Use updated coupled function to eliminate spatial leakage errors completely
                J_ring, x_star, p_list, balance_resid = build_ring_jacobian_heterogeneous_coupled(
                    N_cells, baseline_params, hopping, CV, steady_state_expected)
                if J_ring is None:
                    discarded += 1
                    continue
                
                # Local reaction stability evaluated precisely at the true global steady state slices
                reaction_max = max(np.max(np.real(np.linalg.eigvals(compute_jacobian(x_star[3*i:3*i+3], p_list[i])))) for i in range(N_cells))
            
            # Global network stability with hopping activated
            full_max = np.max(np.real(np.linalg.eigvals(J_ring)))
            
            react_eig.append(reaction_max)
            full_eig.append(full_max)
            residual_log.append(balance_resid)
        ...
        # --- Process Metrics & Statistical Aggregations ---
        n_valid = n_trials - discarded
        rob_cond = 100 * turing_count / n_valid if n_valid > 0 else np.nan
        rob_marg = 100 * turing_count / n_trials
        ...
        results_by_cv.append({
            ...
            'all_reaction': np.array(react_eig), 'all_full': np.array(full_eig)
        })
        
        # Real-time execution print outputs
        print(f"CV={CV:<5} valid={n_valid}/{n_trials} disc={discarded} ({100*discarded/n_trials:.1f}%)")
        if n_valid > 0:
            print(f"    reaction max {np.mean(react_eig):+.4f} (want <0) | full-ring max {np.mean(full_eig):+.4f} (want >0)")
            print(f"    robustness {rob_cond:.1f}% cond / {rob_marg:.1f}% marg  (reactfail {fail_react}, fullstable {fail_stable})")

    # --- Final Formatted Statistical Robustness Summary Table ---
    print("\n" + "=" * 92)
    print(f"{'CV':<6}{'reaction':<12}{'full':<12}{'valid':<8}{'disc%':<8}{'robust(cond)':<14}{'robust(marg)'}")
    print("-" * 92)
    for r in results_by_cv:
