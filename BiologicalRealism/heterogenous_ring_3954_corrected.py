...
WHAT THIS TESTS (the two-condition Turing test, done per Fourier mode):
    Condition A (uniform mode m=0): max Re(lambda) < 0   -> tissue is stable
                                                            without spatial structure
    Condition B (some finite mode m>0): max Re(lambda) > 0 -> diffusion drives a
                                                            finite-wavelength instability
A trial counts as Turing only if BOTH hold. Checking only "does any eigenvalue
go positive" (the OG metric) is wrong: it also fires when the m=0 mode is
unstable, which is a plain (non-Turing) instability of the whole tissue.
...
# ======================================================================
# REACTION KINETICS  (unchanged from Objective 1 -- these are correct)
# ======================================================================
...
def build_ring_jacobian_heterogeneous(N_cells, baseline_params, hopping, CV):
    ...
    ISOLATED reaction fixed point; ring Jacobian = per-cell reaction Jacobians
    + ring diffusion. Returns (J_ring, steady_states, params_list, balance_resid)
    or (None, "no_isolated_ss", None, None) if any cell has no positive fixed point.
    ...
    for CV in [0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4]:
        ...
        for _ in range(n_trials):
            if CV == 0:
                ...
                J_ring, reason, _, bres = build_ring_jacobian_heterogeneous(
            ...
            # validation: how well does the projection capture the true spectrum?
