...
TOPOLOGY 3954: node u DOES self-activate (16 params).
  du = alpha_u + beta_u * H_act(u,K_uu) * H_inh(v,K_vu) - delta_u*u
  (1754 is the same file with this self-activation removed -> 15 params.)
 
METHOD (three fixes over the frozen-coefficient / Fourier version)
------------------------------------------------------------------
1. COUPLED STEADY STATE. Each trial solves the FULL coupled ring steady state
   and linearises there (isolated fixed points are invalid for strong diffusion).
2. ROBUST SOLVER. Damped Newton + fsolve fallback, so a 'discard' means the
   positive coupled base state genuinely does not exist, not solver failure.
3. PROJECTION-FREE TURING TEST (exact for heterogeneous rings):
   ...
   Reduces to the standard dispersion result when cells are identical. The
   Fourier m=0/band split is unreliable under strong heterogeneity (mode mixing)
   and is kept only as a secondary diagnostic.
...
def reaction_max_re(X, params_list, N_cells):
    """maxRe over the block-diagonal (diffusion off): largest local growth rate
    over all cells. Condition A requires this < 0."""
    return max(np.max(np.real(np.linalg.eigvals(compute_jacobian(X[3*i:3*i+3], params_list[i]))))
               for i in range(N_cells))
 
def solve_ring_steady_state(params_list, Ldiff, N_cells, x0, max_iter=200):
    """Damped Newton (backtracking) + fsolve fallback. Returns the positive
    coupled steady state, or None if it genuinely cannot be found."""
    accept = lambda X: (np.max(np.abs(ring_rhs(X, params_list, Ldiff, N_cells))) < 1e-7
                        and np.all(X > 0))
    # 1) damped Newton
    ...
    for _ in range(max_iter):
        ...
        t = 1.0
        for _ls in range(30):
            if np.linalg.norm(ring_rhs(X + t*dX, params_list, Ldiff, N_cells)) < nF:
                break
            t *= 0.5
        X = X + t * dX
    if accept(X):
        return X
    # 2) fsolve fallback with analytic Jacobian, same seed
    Xf, info, ier, msg = fsolve(
        ring_rhs, np.array(x0, dtype=float),
        args=(params_list, Ldiff, N_cells),
        fprime=lambda X, *a: full_ring_jacobian(X, *a),
        full_output=True)
    return Xf if (ier == 1 and accept(Xf)) else None
...
def build_ring_jacobian_heterogeneous(N_cells, baseline_params, hopping, CV, baseline_ss):
    """Draw noisy per-cell params, solve the coupled ring steady state, return
    (J_ring, X, params_list, ring_resid) or (None, 'no_coupled_ss', None, None)."""
